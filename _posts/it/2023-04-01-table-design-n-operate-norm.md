...
1. 【强制】数据库本身库、表、列所有字符集必须保持一致，为`utf8`或`utf8mb4`。优先`utf8mb4`，因为MySQL的`utf8`最大只能存储3字节，导致如emoji字符不能存储，实际表现两者基本一致，都是变长的字符编码（如一个中文字符占用三字节，英文字符占用1字节）。`utf8mb4`只是MySQL中特殊的存在，其他系统没有。
